...
/* Lever A (#34) row size for the FP8-storage attn_comp_kv. Used by the
 * graph-side commit calls, snapshot save/load, and the host-side diagnostic
 * decoder.  Mirrors ds4_gpu_dsv4_fp8_attn_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT):
 *   7 × sizeof(float)    = 28 B per-64-block FP8 scales
 *   + 4 B pad            → 32-byte aligned FP8 region
 *   + 448 uchar FP8 nope = 480-byte boundary
 *   + 64 half RoPE       = 608 bytes total.
 * The macro must be available early because the first use is in graph
 * allocation (storage size) and the host-side decode helper.  Snapshot logic
 * and the comment-only re-mention near the payload version live further down. */
#define DSV4_FP8_ATTN_ROW_BYTES ((uint64_t)608)
...
typedef struct {
    ...
    /* Expert keep-map for offline-shrunken GGUFs.  If `ds4.expert_keep_map.*`
     * metadata is present, the routed expert tensors only carry the kept rows
     * and the router still emits 256-wide logits; the engine masks dropped
     * indices and translates surviving ones from original expert id to the
     * compact tensor slot via `expert_orig_to_compact`.  Memory is tiny: a
     * uint16_t per layer plus an int16_t per (layer, expert) cell. */
    bool       expert_shrunken;
    uint32_t   expert_layer_count;        /* matches kept_counts.length */
    uint16_t  *expert_kept_count;         /* size = expert_layer_count */
    int16_t   *expert_orig_to_compact;    /* size = expert_layer_count * DS4_N_EXPERT, -1 for dropped */
} ds4_model;
...
/* Returns the routed-expert slot count for a layer (per-model, per-layer).
 * For an unshrunken GGUF this is always DS4_N_EXPERT; for a shrunken one it
 * comes from ds4.expert_keep_map.kept_counts. */
...
/* If the GGUF was produced by gguf-tools/shrink_gguf.py, parse the two
 * keep-map arrays into the model.  Absence is fine — the model just runs
 * with the full 256 routed experts.  Called from model_open(). */
static void load_expert_keep_map(ds4_model *m) {
    ...
    /* Initialize to -1 (== "dropped"); kept entries get overwritten below. */
    for (size_t i = 0; i < (size_t)m->expert_layer_count * DS4_N_EXPERT; i++) {
        m->expert_orig_to_compact[i] = -1;
    }
...
/* Lever A: host-side decode of a 608-byte FP8+scale attn_comp_kv row into
 * float[head_dim]. Mirrors the layout of kernel_dsv4_f32_to_fp8_store_rows:
 *   bytes 0..27      = 7 × float32 per-64-block scales
 *   bytes 28..31     = pad to 8-byte alignment
 *   bytes 32..479    = 448 × uchar FP8 (E4M3FN, sign|magnitude)
 *   bytes 480..607   = 64 × half RoPE
 * Used by the prompt-graph diagnostic at the bottom of the Metal path, which
 * compares the GPU compressed-KV cache to the CPU reference. */
static void dsv4_fp8_attn_row_decode_cpu(const uint8_t *src, float *dst,
                                         uint32_t head_dim, uint32_t n_rot) {
    const uint32_t n_nope   = head_dim - n_rot;
    const uint32_t n_blocks = n_nope >> 6;
    const uint32_t scales_bytes = n_blocks * (uint32_t)sizeof(float);
    const uint32_t nope_off = (scales_bytes + 7u) & ~7u;
    const uint32_t rope_off = nope_off + n_nope;

    const float    *scales = (const float *)src;
    const uint8_t  *nope   = src + nope_off;
    const uint16_t *rope   = (const uint16_t *)(src + rope_off);

    for (uint32_t b = 0; b < n_blocks; b++) {
        const float s = scales[b];
        for (uint32_t i = 0; i < 64; i++) {
            const uint8_t byte = nope[(b << 6) + i];
            const float mag = dsv4_e4m3fn_value_cpu((int)(byte & 0x7f));
            const float v = (byte & 0x80) ? -mag : mag;
            dst[(b << 6) + i] = v * s;
        }
    }
    for (uint32_t i = 0; i < n_rot; i++) {
        dst[n_nope + i] = f16_to_f32(rope[i]);
    }
}
...
static void weights_validate_layout(const ds4_model *m, const ds4_weights *w) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        /* For a shrunken GGUF the routed expert tensors only carry the kept
         * rows; router weight + bias still emit 256-wide logits, so the
         * checks above stay the same.  expert_count below comes from the
         * keep-map metadata when present, otherwise = DS4_N_EXPERT. */
...
static void mtp_weights_validate_layout(const ds4_model *m, const ds4_mtp_weights *w) {
    const uint64_t hc_dim = (uint64_t)DS4_N_EMBD * DS4_N_HC;
    const uint64_t hc_mix_dim = 2u * DS4_N_HC + (uint64_t)DS4_N_HC * DS4_N_HC;
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const uint64_t out_low_dim = (uint64_t)DS4_N_OUT_GROUP * DS4_N_LORA_O;
    ...
    /* MTP block 0 — same shrink semantics as the main blocks. */
    const uint64_t exp_dim = model_expert_kept_count(m, 0);
    tensor_expect_routed_expert(l->ffn_gate_exps, 3, DS4_N_EMBD, DS4_N_FF_EXP, exp_dim);
    tensor_expect_routed_expert(l->ffn_up_exps,   3, DS4_N_EMBD, DS4_N_FF_EXP, exp_dim);
    tensor_expect_routed_expert(l->ffn_down_exps, 3, DS4_N_FF_EXP, DS4_N_EMBD, exp_dim);
    if (l->ffn_gate_exps->type != l->ffn_up_exps->type) {
...
static void mtp_weights_bind(ds4_mtp_weights *w, const ds4_model *m) {
    ...
    mtp_weights_validate_layout(m, w);
}
...
/* Layer-aware variant: takes the ORIGINAL expert id (0..DS4_N_EXPERT-1) and
 * looks up the compact tensor slot for shrunken GGUFs.  On non-shrunken
 * models the original id is the tensor slot, so this is a free pass-through.
 *
 * The engine guarantees (via its auto-mask built from the same keep-map)
 * that the router never selects a dropped expert.  If it does, that's a
 * loader bug and we abort rather than silently misroute. */
static const uint8_t *tensor_expert_bytes_il(
        const ds4_model  *m,
        const ds4_tensor *w,
        uint32_t          il,
        uint32_t          orig_expert,
        uint64_t         *in_dim,
        uint64_t         *out_dim,
        uint64_t         *row_bytes) {
    uint32_t slot = orig_expert;
    if (m->expert_orig_to_compact && il < m->expert_layer_count) {
        if (orig_expert >= DS4_N_EXPERT) {
            ds4_die("router emitted an expert id outside 0..255");
        }
        const int16_t s = m->expert_orig_to_compact[(size_t)il * DS4_N_EXPERT + orig_expert];
        if (s < 0) {
            ds4_die("router selected a dropped expert (auto-mask should have blocked this)");
        }
        slot = (uint32_t)s;
    }
    return tensor_expert_bytes(m, w, slot, in_dim, out_dim, row_bytes);
}
...
static void matvec_iq2_xxs_expert_pair_prequant(
    ...
    const uint8_t *base0 = tensor_expert_bytes_il(m, w0, il, expert, &in_dim0, &out_dim0, &row_bytes0);
    const uint8_t *base1 = tensor_expert_bytes_il(m, w1, il, expert, &in_dim1, &out_dim1, &row_bytes1);
...
static void matvec_iq2_xxs_experts_mid_prequant(
    ...
    for (int i = 0; i < n_expert; i++) {
        ...
        ctx.gate_base[i] = tensor_expert_bytes_il(m, gate_w, il, (uint32_t)selected[i],
                                               &gate_in_dim, &gate_out_dim, &ctx.gate_row_bytes[i]);
        ctx.up_base[i] = tensor_expert_bytes_il(m, up_w, il, (uint32_t)selected[i],
...
static void matvec_q2_k_expert(
    ...
    const uint8_t *base = tensor_expert_bytes_il(m, w, il, expert, &in_dim, &out_dim, &row_bytes);
...
static void matvec_q2_k_experts_accum_prequant(
    ...
    for (int i = 0; i < n_expert; i++) {
        ...
        base[i] = tensor_expert_bytes_il(m, w, il, (uint32_t)selected[i], &in_dim, &out_dim, &row_bytes[i]);
        if (i == 0) {
            in_dim0 = in_dim;
            out_dim0 = out_dim;
...
/* Dot product where one operand is F16 storage (compressed KV cache rows).  The
 * KV rows are already F16-quantized at write time, so reading via f16_to_f32 is
 * bit-equal to the previous F32-storage path that ran f16_to_f32(f32_to_f16())
 * on each store. */
static inline float dot_f16_f32(const uint16_t *a, const float *b, uint32_t n) {
#if defined(__ARM_NEON)
    uint32_t i = 0;
    float32x4_t acc0 = vdupq_n_f32(0.0f);
    float32x4_t acc1 = vdupq_n_f32(0.0f);
    for (; i + 8 <= n; i += 8) {
        const float16x4_t h0 = vreinterpret_f16_u16(vld1_u16(a + i));
        const float16x4_t h1 = vreinterpret_f16_u16(vld1_u16(a + i + 4));
        acc0 = vfmaq_f32(acc0, vcvt_f32_f16(h0), vld1q_f32(b + i));
        acc1 = vfmaq_f32(acc1, vcvt_f32_f16(h1), vld1q_f32(b + i + 4));
    }
    float acc = vaddvq_f32(vaddq_f32(acc0, acc1));
    for (; i < n; i++) acc += f16_to_f32(a[i]) * b[i];
    return acc;
#else
    float acc = 0.0f;
    for (uint32_t i = 0; i < n; i++) acc += f16_to_f32(a[i]) * b[i];
    return acc;
#endif
}

static inline void axpy_f16_f32(float *y, const uint16_t *x, float a, uint32_t n) {
#if defined(__ARM_NEON)
    uint32_t i = 0;
    const float32x4_t av = vdupq_n_f32(a);
    for (; i + 8 <= n; i += 8) {
        const float16x4_t h0 = vreinterpret_f16_u16(vld1_u16(x + i));
        const float16x4_t h1 = vreinterpret_f16_u16(vld1_u16(x + i + 4));
        vst1q_f32(y + i,     vfmaq_f32(vld1q_f32(y + i),     av, vcvt_f32_f16(h0)));
        vst1q_f32(y + i + 4, vfmaq_f32(vld1q_f32(y + i + 4), av, vcvt_f32_f16(h1)));
    }
    for (; i < n; i++) y[i] += a * f16_to_f32(x[i]);
#else
    for (uint32_t i = 0; i < n; i++) y[i] += a * f16_to_f32(x[i]);
#endif
}
...
/* Single live engine at a time (instance lock); router top-K reads this without
 * passing the engine through every inner loop. */
static const uint8_t *g_cpu_expert_mask = NULL;
...
static void layer_topk_selected_experts_from_probs(
    ...
    if (g_cpu_expert_mask && il < DS4_N_LAYER) {
        const uint8_t *row = g_cpu_expert_mask + (size_t)il * DS4_N_EXPERT;
        for (int i = 0; i < (int)DS4_N_EXPERT; i++) {
            if (!row[i]) selection[i] = -FLT_MAX;
        }
    }
...
static void layer_routed_moe_one(
    ...
    if (!trace) {
        ...
        matvec_q2_k_experts_accum_prequant(out, model, layer->ffn_down_exps, midq, selected, DS4_N_EXPERT_USED, il);
    } else {
        for (uint32_t i = 0; i < DS4_N_EXPERT_USED; i++) {
            ...
            matvec_q2_k_expert(down, model, layer->ffn_down_exps, mid, expert, il);
...
static void layer_routed_moe_batch(
    ...
    for (uint32_t ai = 0; ai < n_active; ai++) {
        ...
        uint64_t gate_in_dim, gate_out_dim;
        uint64_t up_in_dim, up_out_dim;
        mid_ctx.gate_base[e] = tensor_expert_bytes_il(model, layer->ffn_gate_exps, il, e,
                                                   &gate_in_dim, &gate_out_dim, &mid_ctx.gate_row_bytes[e]);
        mid_ctx.up_base[e] = tensor_expert_bytes_il(model, layer->ffn_up_exps, il, e,
    ...
    for (uint32_t ai = 0; ai < n_active; ai++) {
        const uint32_t e = active_expert[ai];
        uint64_t in_dim, out_dim;
        down_ctx.base[e] = tensor_expert_bytes_il(model, layer->ffn_down_exps, il, e,
...
typedef struct {
    ...
    uint32_t comp_cap;
    uint32_t n_comp;
    /* Compressed KV rows are stored in F16 to halve the 1M-context KV footprint.
     * Math upstream/downstream is unchanged: writes already F16-rounded the rows
     * before storing them as F32, and the GPU kernel already cast to half on
     * load. */
    uint16_t *attn_comp_kv;
    float *attn_state_kv;
    float *attn_state_score;

    uint32_t n_index_comp;
    uint16_t *index_comp_kv;
...
static void kv_cache_init(ds4_kv_cache *cache, uint32_t ctx_size, uint32_t raw_cap) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (ratio != 0) {
            ...
            cache->layer[il].attn_comp_kv = xmalloc_zeroed((size_t)comp_cap * DS4_N_HEAD_DIM, sizeof(uint16_t));
            ...
            if (ratio == 4) {
                ...
                cache->layer[il].index_comp_kv = xmalloc_zeroed((size_t)comp_cap * DS4_N_INDEXER_HEAD_DIM, sizeof(uint16_t));
...
static void kv_cache_push_comp(uint16_t *rows, uint32_t *n_rows, uint32_t cap_rows, uint32_t row_dim, const float *kv) {
    if (*n_rows >= cap_rows) ds4_die("compressed KV cache capacity exceeded");
    uint16_t *dst = rows + (uint64_t)(*n_rows) * row_dim;
    for (uint32_t i = 0; i < row_dim; i++) dst[i] = f32_to_f16(kv[i]);
    (*n_rows)++;
}
...
static void layer_attention_mixed_one(
    ...
    for (uint32_t h = 0; h < DS4_N_HEAD; h++) {
        ...
        for (uint32_t r = 0; r < n_comp; r++, idx++) {
            ...
            const uint16_t *kv = comp_kv + (uint64_t)r * DS4_N_HEAD_DIM;
            score[idx] = dot_f16_f32(kv, qh, DS4_N_HEAD_DIM) * kq_scale;
            if (score[idx] > max_score) max_score = score[idx];
        }
        ...
        for (uint32_t r = 0; r < n_comp; r++, idx++) {
            ...
            const uint16_t *kv = comp_kv + (uint64_t)r * DS4_N_HEAD_DIM;
            denom += weight;
            axpy_f16_f32(oh, kv, weight, DS4_N_HEAD_DIM);
        }
...
static void layer_attention_prefix_batch_worker(void *vctx, uint64_t r0, uint64_t r1) {
    ...
    for (uint64_t idx = r0; idx < r1; idx++) {
        ...
        for (uint32_t c = 0; c < comp_count; c++, sidx++) {
            ...
            const uint16_t *kv = ctx->comp_kv + (uint64_t)c * DS4_N_HEAD_DIM;
            score[sidx] = dot_f16_f32(kv, qh, DS4_N_HEAD_DIM) * kq_scale;
            if (score[sidx] > max_score) max_score = score[sidx];
        }
...
static bool *indexer_allowed_decode_one(
    ...
    for (uint32_t c = 0; c < n_comp; c++) {
        const uint16_t *kv = index_comp + (uint64_t)c * head_dim;
        float s = 0.0f;
        for (uint32_t h = 0; h < n_head; h++) {
            const float *qh = q + (uint64_t)h * head_dim;
            float dot = dot_f16_f32(kv, qh, head_dim);
...
static void layer_attention_raw_swa_batch(
    ...
    if (prefix_batch_attn) {
        double tx = profile ? now_sec() : 0.0;
        /* Compressed-KV buffer is only dereferenced when comp_counts is non-zero,
         * which only happens for layers with attn_comp_kv allocated.  Passing it
         * through directly (possibly NULL for compress_ratio==0 layers) is safe. */
        const uint16_t *comp_kv_for_prefix = cache->attn_comp_kv;
        if (!heads) {
            heads = xmalloc((size_t)n_tok * q_dim * sizeof(heads[0]));
        }
...
typedef struct {
    ...
    /* F32 scratch slab the producer pipeline writes into before the final
     * F32→F16 store commits values to layer_*_comp_cache.  Sized for the worst
     * case of one full prefill chunk × max(HEAD_DIM, INDEXER_HEAD_DIM).  Reused
     * across both attn and indexer paths since they run serially. */
    ds4_gpu_tensor *comp_scratch_f32;
...
static void metal_graph_free(ds4_gpu_graph *g) {
    ...
    ds4_gpu_tensor_free(g->comp_scratch_f32);
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
...
static bool metal_graph_alloc_raw_cap(
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (ratio != 0) {
            ...
            /* Lever A: attn_comp_kv rows are 608-byte FP8+scale packs, not
             * head_dim F16. Producer kernels (f32_to_fp8_store_rows) write
             * this layout; consumer FA paths unpack it via dsv4_fp8_rows_to_f16
             * pack-in, and heads8/heads8_rb4 read it inline. */
            g->layer_attn_comp_cache[il] = metal_graph_alloc_kv_cache_tensor(
                    managed_kv_cache,
                    (uint64_t)g->layer_comp_cap[il] * DSV4_FP8_ATTN_ROW_BYTES);
            ...
            if (ratio == 4) {
                ...
                g->layer_index_comp_cache[il] = metal_graph_alloc_kv_cache_tensor(
                        managed_kv_cache,
                        (uint64_t)g->layer_comp_cap[il] * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t));
    ...
    /* F32 scratch for the producer pipeline before F16 commit.  Worst case is
     * a full prefill chunk landing in the indexer or attn cache at once; both
     * paths can share the same buffer because they never run concurrently. */
    {
        const uint64_t scratch_head = (uint64_t)DS4_N_HEAD_DIM > (uint64_t)DS4_N_INDEXER_HEAD_DIM
                                          ? (uint64_t)DS4_N_HEAD_DIM
                                          : (uint64_t)DS4_N_INDEXER_HEAD_DIM;
        const uint64_t scratch_rows = pc != 0 ? (uint64_t)pc : 1u;
        g->comp_scratch_f32 = ds4_gpu_tensor_alloc(scratch_rows * scratch_head * sizeof(float));
    }
...
#define DS4_METAL_PROFILE_DECODE_STAGE(name) do { \
    ...
    if (ok && compressed) {
        ...
        ds4_gpu_tensor *attn_scratch_view = ds4_gpu_tensor_view(
                g->comp_scratch_f32,
                0,
                (uint64_t)DS4_N_HEAD_DIM * sizeof(float));
        if (ok && !attn_scratch_view) ok = false;
        if (ok) ok = ds4_gpu_compressor_update_tensor(g->comp_kv_cur,
                                                        ...
                                                        DS4_RMS_EPS) != 0;
        if (ok && emit) {
            /* Lever A: the f32->fp8 store kernel does its own per-64-block FP8
             * encoding and writes the 608-byte row directly, so the prior
             * "FP8 round-trip in F32 scratch, then F16 commit" pair is now
             * a single store call. The debug dump still inspects the F32
             * source row (the cache row is opaque FP8+scale after this). */
            metal_graph_debug_dump_tensor("KVcompress", attn_scratch_view, DS4_N_HEAD_DIM, il, pos);
            ok = ds4_gpu_dsv4_f32_to_fp8_store_rows_tensor(
                                    g->layer_attn_comp_cache[il],
                                    (uint64_t)comp_row * DSV4_FP8_ATTN_ROW_BYTES,
                                    g->comp_scratch_f32,
                                    0,
                                    1,
                                    DS4_N_HEAD_DIM,
        ...
        ds4_gpu_tensor_free(attn_scratch_view);
        ...
        if (ok && ratio == 4) {
            ...
            ds4_gpu_tensor *index_scratch_view = ds4_gpu_tensor_view(
                    g->comp_scratch_f32,
                    0,
                    (uint64_t)DS4_N_INDEXER_HEAD_DIM * sizeof(float));
            if (ok && !index_scratch_view) ok = false;
            if (ok) ok = ds4_gpu_compressor_update_tensor(g->comp_kv_cur,
                                                            g->comp_sc_cur,
                                                            ...
                                                            DS4_RMS_EPS) != 0;
            if (ok && emit) {
                ok = ds4_gpu_dsv4_indexer_qat_tensor(index_scratch_view,
                                                      1,
                                                      DS4_N_INDEXER_HEAD_DIM) != 0;
                if (ok) ok = ds4_gpu_dsv4_f32_to_f16_store_rows_tensor(
                                        g->layer_index_comp_cache[il],
                                        (uint64_t)index_row * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t),
                                        g->comp_scratch_f32,
                                        0,
                                        1,
                                        DS4_N_INDEXER_HEAD_DIM) != 0;
            }
            ds4_gpu_tensor_free(index_scratch_view);
...
static int metal_graph_decode_test(
    ...
    if (layer->ffn_gate_tid2eid) {
        ...
        layer_topk_selected_experts(selected, expert_weight, model, layer, 0, cpu_ffn_norm);
    }
    for (uint32_t i = 0; i < DS4_N_EMBD; i++) cpu_ffn_out[i] = cpu_shared[i] + cpu_routed[i];
...
#define DS4_METAL_PROFILE_Q_STAGE(name) do { \
    ...
    if (ok && raw_batch_attention) {
        ...
        if (zero_prefix) {
            ...
            if (ok) {
                ds4_gpu_tensor *attn_scratch_view = NULL;
                if (n_comp != 0) {
                    attn_scratch_view = ds4_gpu_tensor_view(
                            g->comp_scratch_f32,
                            0,
                            (uint64_t)n_comp * DS4_N_HEAD_DIM * sizeof(float));
                    if (!attn_scratch_view) ok = false;
                }
                /* Lever A: quantize_fp8=false skips the redundant F32 scratch
                 * round-trip; f32_to_fp8_store_rows below does the real FP8
                 * encoding while writing the 608-byte cache rows. */
                if (ok) ok = ds4_gpu_compressor_prefill_tensor(n_comp ? attn_scratch_view : g->comp_scratch_f32,
                                                         g->layer_attn_state_kv[il],
                                                         g->layer_attn_state_score[il],
                                                         g->batch_comp_kv,
                                                         g->batch_comp_sc,
            ...
            if (ok) {
                ...
                if (n_comp != 0) {
                    /* Lever A: attn_comp_cache is now opaque FP8+scale; the
                     * dump captures the F32 source rows from comp_scratch_f32
                     * before the FP8 encode-and-commit ran above. */
            ...
            if (aligned_chunk) {
                ...
                if (ok && comp_chunk != 0) {
                    comp_view = ds4_gpu_tensor_view(g->comp_scratch_f32,
                                                      0,
                                                      (uint64_t)comp_chunk * DS4_N_HEAD_DIM * sizeof(float));
                    ok = comp_view != NULL;
                }
                /* Lever A: quantize_fp8=false; FP8 encoding now happens inside
                 * the f32->fp8 store kernel that writes the 608-byte cache
                 * rows below. */
                if (ok && ratio == 4) {
                    ok = ds4_gpu_compressor_prefill_ratio4_replay_tensor(
                            comp_view ? comp_view : g->comp_scratch_f32,
                            g->layer_attn_state_kv[il],
                            g->layer_attn_state_score[il],
                            g->batch_comp_kv,
                            g->batch_comp_sc,
                ...
                if (ok && comp_chunk != 0) {
                    ok = ds4_gpu_dsv4_f32_to_fp8_store_rows_tensor(
                                            g->layer_attn_comp_cache[il],
                                            (uint64_t)comp_before * DSV4_FP8_ATTN_ROW_BYTES,
                                            g->comp_scratch_f32,
                                            0,
                                            comp_chunk,
                                            DS4_N_HEAD_DIM,
                ...
                for (uint32_t t = 0; ok && t < n_tokens; t++) {
                    ...
                    ds4_gpu_tensor *attn_scratch_view = ds4_gpu_tensor_view(
                            ...
                            0,
                            (uint64_t)DS4_N_HEAD_DIM * sizeof(float));
                    ok = kv_view && sc_view && attn_scratch_view &&
                         ds4_gpu_compressor_update_tensor(kv_view,
                                                            ...
                                                            DS4_RMS_EPS) != 0;
                    if (ok && emit) {
                        /* Lever A: the f32->fp8 store handles its own per-block
                         * FP8 encoding while writing the 608-byte row, so the
                         * prior in-scratch round-trip is no longer needed. */
                        metal_graph_debug_dump_tensor("KVcompress",
                                                      attn_scratch_view,
                                                      DS4_N_HEAD_DIM,
                                                      il,
        ...
        if (ok && ratio == 4) {
            ...
            if (zero_prefix) {
                ...
                ds4_gpu_tensor *index_scratch_view = NULL;
                if (ok && n_comp != 0) {
                    index_scratch_view = ds4_gpu_tensor_view(
                            g->comp_scratch_f32,
                            0,
                            (uint64_t)n_comp * DS4_N_INDEXER_HEAD_DIM * sizeof(float));
                    if (!index_scratch_view) ok = false;
                }
                if (ok) {
                    ok = ds4_gpu_compressor_prefill_tensor(index_scratch_view ? index_scratch_view : g->comp_scratch_f32,
                                                             g->layer_index_state_kv[il],
                                                             g->layer_index_state_score[il],
                                                             g->batch_comp_kv,
                                                             g->batch_comp_sc,
                ...
                if (ok && n_comp != 0) {
                    ok = ds4_gpu_dsv4_f32_to_f16_store_rows_tensor(
                                            g->layer_index_comp_cache[il],
                                            0,
                                            g->comp_scratch_f32,
                                            0,
                ...
                if (aligned_chunk) {
                    ...
                    if (ok) {
                        ok = ds4_gpu_compressor_prefill_ratio4_replay_tensor(
                                index_view ? index_view : g->comp_scratch_f32,
                                g->layer_index_state_kv[il],
                                g->layer_index_state_score[il],
                                g->batch_comp_kv,
                                g->batch_comp_sc,
                    ...
                    if (ok && index_chunk != 0) {
                        ok = ds4_gpu_dsv4_f32_to_f16_store_rows_tensor(
                                                g->layer_index_comp_cache[il],
                                                (uint64_t)index_before * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t),
                                                g->comp_scratch_f32,
                                                0,
                                                index_chunk,
                                                DS4_N_INDEXER_HEAD_DIM) != 0;
                    ...
                    for (uint32_t t = 0; ok && t < n_tokens; t++) {
                        ...
                        ds4_gpu_tensor *index_scratch_view = ds4_gpu_tensor_view(
                                ...
                                0,
                                (uint64_t)DS4_N_INDEXER_HEAD_DIM * sizeof(float));
                        ok = kv_view && sc_view && index_scratch_view &&
                             ds4_gpu_compressor_update_tensor(kv_view,
                                                                sc_view,
                                                                g->layer_index_state_kv[il],
                                                                g->layer_index_state_score[il],
...
static bool metal_graph_prefill_layer_major(
     ...
     * server gets regular scheduling points.  Also split non-tiny prefills
     * when a frontend asked for display progress: completed layer command
     * buffers are real scheduling/keepalive points, while callbacks emitted
     * while encoding one huge command buffer would only be cosmetic.
     *
...
ds4_context_memory ds4_context_memory_estimate(ds4_backend backend, int ctx_size) {
    ...
    if (ds4_backend_uses_graph(backend)) {
        ...
        m.raw_bytes = (uint64_t)DS4_N_LAYER *
                      ...
                      DS4_N_HEAD_DIM *
                      sizeof(float);
        /* Metal compressed-KV buffers still hold F32 rows; the F16 storage
         * conversion has only landed on the CPU reference path so far.  Once
         * the GPU kernels (compressor write + indexed attention read) move to
         * half-precision rows this estimate must drop to sizeof(uint16_t). */
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            const uint32_t ratio = ds4_layer_compress_ratio(il);
            if (ratio == 0) continue;
    ...
    if (ok) {
        ...
        if (getenv("DS4_METAL_GRAPH_TRACE_CACHE") != NULL ||
            ...
            for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
                ...
                /* Lever A: GPU comp cache storage is 608-byte FP8+scale rows.
                 * Read raw bytes, decode each row to float on the host, and
                 * compare against the CPU reference's F16 attn_comp_kv widened
                 * to float. */
                const size_t row_bytes = (size_t)DSV4_FP8_ATTN_ROW_BYTES;
                uint8_t *gpu_comp_raw = xmalloc((size_t)n_comp * row_bytes);
                if (ds4_gpu_tensor_read(g.layer_attn_comp_cache[il], 0,
                    ...
                    float *cpu_comp_f32 = xmalloc((size_t)n * sizeof(float));
                    for (uint32_t r = 0; r < n_comp; r++) {
                        dsv4_fp8_attn_row_decode_cpu(
                                gpu_comp_raw + (size_t)r * row_bytes,
                                gpu_comp + (uint64_t)r * DS4_N_HEAD_DIM,
                                DS4_N_HEAD_DIM, DS4_N_ROT);
                    }
                    for (uint64_t i = 0; i < n; i++) {
                        cpu_comp_f32[i] = f16_to_f32(cpu_cache.layer[il].attn_comp_kv[i]);
                    }
                    fprintf(stderr,
                            "ds4: comp trace layer %u n=%u attn_max=%g attn_rms=%g\n",
                            il, n_comp,
                            max_abs_diff(cpu_comp_f32, gpu_comp, n),
                            rms_abs_diff(cpu_comp_f32, gpu_comp, n));
                    free(cpu_comp_f32);
                    free(gpu_comp);
                }
                free(gpu_comp_raw);
                ...
                if (n_index != 0 && g.layer_index_comp_cache[il]) {
                    ...
                    uint16_t *gpu_index_f16 = xmalloc((size_t)ni * sizeof(uint16_t));
                    if (ds4_gpu_tensor_read(g.layer_index_comp_cache[il], 0, gpu_index_f16, ni * sizeof(uint16_t)) != 0) {
                        float *gpu_index = xmalloc((size_t)ni * sizeof(float));
                        float *cpu_index_f32 = xmalloc((size_t)ni * sizeof(float));
                        for (uint64_t i = 0; i < ni; i++) {
                            gpu_index[i] = f16_to_f32(gpu_index_f16[i]);
                            cpu_index_f32[i] = f16_to_f32(cpu_cache.layer[il].index_comp_kv[i]);
                        }
                        fprintf(stderr,
                                "ds4: comp trace layer %u n=%u index_max=%g index_rms=%g\n",
                                il, n_index,
                                max_abs_diff(cpu_index_f32, gpu_index, ni),
                                rms_abs_diff(cpu_index_f32, gpu_index, ni));
                        free(cpu_index_f32);
                        free(gpu_index);
                    }
                    free(gpu_index_f16);
...
struct ds4_engine {
    ...
    float directional_steering_attn_scale;
    float directional_steering_ffn_scale;
    char *expert_mask_file;
    uint8_t *expert_mask; /* size DS4_N_LAYER * DS4_N_EXPERT, 1=keep 0=disable */
    bool quality;
    bool metal_ready;
    bool mtp_ready;
};
...
/* DSXM expert mask: magic="DSXM" u8[4], version u32 (=1), n_layers u32,
 * n_expert u32, reserved u32, bits ceil(n_layers*n_expert/8) bytes.
 * bit (layer*n_expert + expert) == 1 means KEEP. */
static bool load_expert_mask(ds4_engine *e) {
    if (!e) return true;
    const bool have_file = e->expert_mask_file && e->expert_mask_file[0];
    const bool shrunken  = e->model.expert_shrunken;
    if (!have_file && !shrunken) return true;

    const size_t total_bits = (size_t)DS4_N_LAYER * DS4_N_EXPERT;

    if (have_file) {
        const char *path = e->expert_mask_file;
        FILE *fp = fopen(path, "rb");
        if (!fp) {
            fprintf(stderr, "ds4: failed to open expert mask %s: %s\n", path, strerror(errno));
            return false;
        }
        uint8_t magic[4];
        uint32_t version = 0, n_layers = 0, n_expert = 0, reserved = 0;
        bool ok = fread(magic, 1, 4, fp) == 4 &&
                  fread(&version, sizeof(version), 1, fp) == 1 &&
                  fread(&n_layers, sizeof(n_layers), 1, fp) == 1 &&
                  fread(&n_expert, sizeof(n_expert), 1, fp) == 1 &&
                  fread(&reserved, sizeof(reserved), 1, fp) == 1;
        if (!ok || memcmp(magic, "DSXM", 4) != 0) {
            fprintf(stderr, "ds4: expert mask %s: bad header\n", path);
            fclose(fp);
            return false;
        }
        if (version != 1) {
            fprintf(stderr, "ds4: expert mask %s: unsupported version %u\n", path, version);
            fclose(fp);
            return false;
        }
        if (n_layers != DS4_N_LAYER || n_expert != DS4_N_EXPERT) {
            fprintf(stderr, "ds4: expert mask %s: shape mismatch (file %ux%u, model %ux%u)\n",
                    path, n_layers, n_expert, DS4_N_LAYER, DS4_N_EXPERT);
            fclose(fp);
            return false;
        }
        const size_t n_bytes = (total_bits + 7) / 8;
        uint8_t *bits = xmalloc(n_bytes);
        if (fread(bits, 1, n_bytes, fp) != n_bytes) {
            fprintf(stderr, "ds4: expert mask %s: short read\n", path);
            free(bits);
            fclose(fp);
            return false;
        }
        ...
        e->expert_mask = xmalloc(total_bits);
        for (size_t i = 0; i < total_bits; i++) {
            e->expert_mask[i] = (bits[i >> 3] >> (i & 7)) & 1u;
        }
        free(bits);
    } else {
        /* No user mask but the GGUF is shrunken: start with all-kept and let
         * the keep_map fold zeros in below. */
        e->expert_mask = xmalloc(total_bits);
        memset(e->expert_mask, 1, total_bits);
    }

    /* AND-combine with the shrunken GGUF's implicit keep_map. A dropped expert
     * in the model has no tensor data on disk; routing to it is fatal, so we
     * force it to 0 regardless of what the user mask said. */
    if (shrunken) {
        const uint32_t lc = e->model.expert_layer_count;
        for (uint32_t il = 0; il < DS4_N_LAYER && il < lc; il++) {
            const int16_t *row = e->model.expert_orig_to_compact + (size_t)il * DS4_N_EXPERT;
            uint8_t *mrow = e->expert_mask + (size_t)il * DS4_N_EXPERT;
            for (uint32_t ex = 0; ex < DS4_N_EXPERT; ex++) {
                if (row[ex] < 0) mrow[ex] = 0;
            }
        }
    }

    int kept = 0;
    for (size_t i = 0; i < total_bits; i++) kept += e->expert_mask[i];
    g_cpu_expert_mask = e->expert_mask;
    if (have_file && shrunken) {
        fprintf(stderr, "ds4: expert mask loaded: %s AND keep_map  kept=%d/%zu (%.1f%%)\n",
                e->expert_mask_file, kept, total_bits,
                100.0 * (double)kept / (double)total_bits);
    } else if (have_file) {
        fprintf(stderr, "ds4: expert mask loaded: %s  kept=%d/%zu (%.1f%%)\n",
                e->expert_mask_file, kept, total_bits,
                100.0 * (double)kept / (double)total_bits);
    } else {
        fprintf(stderr, "ds4: expert mask synthesized from shrunken-GGUF keep_map  kept=%d/%zu (%.1f%%)\n",
                kept, total_bits,
                100.0 * (double)kept / (double)total_bits);
    }
    return true;
...
/* v2: compressed-KV rows serialized as F16 (half their v1 byte width).
 * v3: Metal-side comp/index_comp cache storage flipped to F16 — snapshot bytes
 *     written/read are now uint16_t per element on the GPU path, matching what
 *     the CPU side already stored on disk. v2 files were written with twice as
 *     many bytes per row from the GPU and cannot be loaded by a v3 runtime.
 * v4: Lever A — Metal attn_comp_kv storage flipped from F16 to FP8 (E4M3FN)
 *     with per-64-element block scales. Each attn comp row is now
 *     DSV4_FP8_ATTN_ROW_BYTES (608) instead of head_dim*2 (1024).
 *     index_comp_kv remains F16 in v4 (that's Lever B). v3 GPU snapshots
 *     intentionally cannot be loaded by a v4 runtime. */
#define DS4_SESSION_PAYLOAD_VERSION UINT32_C(4)

/* DSV4_FP8_ATTN_ROW_BYTES is defined near the top of the file (after the
 * DS4_N_* enum block) because the earliest uses are in graph allocation and
 * the host-side decode helper. */
...
static uint64_t session_payload_live_tensor_bytes(const ds4_gpu_graph *g, uint32_t checkpoint_len) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        const uint32_t ratio = ds4_layer_compress_ratio(il);
        if (ratio == 0) continue;
        /* Lever A (#34): GPU attn_comp_kv is the 608-byte FP8+scale+F16-RoPE row,
         * not F16 (DS4_N_HEAD_DIM * 2 bytes).  The live save and load below mirror
         * this exact layout, so the budget must match. */
        bytes += (uint64_t)g->layer_n_comp[il] * DSV4_FP8_ATTN_ROW_BYTES;
        bytes += layer_attn_state_bytes(ratio);
        bytes += layer_attn_state_bytes(ratio);
        if (ratio == 4) {
            /* Indexer comp_kv remains F16 in Lever A (Lever B is the indexer-side
             * FP8 follow-up). */
            bytes += (uint64_t)g->layer_n_index_comp[il] * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t);
...
static uint64_t session_cpu_payload_live_tensor_bytes(const ds4_session *s) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        const uint32_t ratio = layer->compress_ratio;
        if (ratio == 0) continue;
        bytes += (uint64_t)layer->n_comp * DS4_N_HEAD_DIM * sizeof(uint16_t);
        bytes += layer_attn_state_bytes(ratio);
        bytes += layer_attn_state_bytes(ratio);
        if (ratio == 4) {
            bytes += (uint64_t)layer->n_index_comp * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t);
            bytes += layer_index_state_bytes(ratio);
            bytes += layer_index_state_bytes(ratio);
        }
    }
...
int ds4_session_save_payload(ds4_session *s, FILE *fp, char *err, size_t errlen) {
    ...
    if (ds4_session_is_cpu(s)) {
        ...
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            ...
            if (payload_write_bytes(fp,
                                    layer->attn_comp_kv,
                                    (uint64_t)layer->n_comp * DS4_N_HEAD_DIM * sizeof(uint16_t),
                                    err,
                                    errlen) != 0) return 1;
            ...
            if (ratio == 4) {
                if (payload_write_bytes(fp,
                                        layer->index_comp_kv,
                                        (uint64_t)layer->n_index_comp * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t),
                                        err,
                                        errlen) != 0) return 1;
    ...
    for (uint32_t il = 0; rc == 0 && il < DS4_N_LAYER; il++) {
        ...
        /* Lever A (#34): attn_comp_cache rows are 608 bytes of FP8+scale+F16-RoPE,
         * not F16 head-dim rows. */
        rc = payload_write_tensor_span(fp,
                                       g->layer_attn_comp_cache[il],
                                       0,
                                       (uint64_t)g->layer_n_comp[il] * DSV4_FP8_ATTN_ROW_BYTES,
                                       buf,
                                       DS4_SESSION_IO_CHUNK,
                                       err,
                                       errlen);
        ...
        if (rc == 0 && ratio == 4) {
            rc = payload_write_tensor_span(fp,
                                           g->layer_index_comp_cache[il],
                                           0,
                                           (uint64_t)g->layer_n_index_comp[il] * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t),
                                           buf,
                                           DS4_SESSION_IO_CHUNK,
                                           err,
                                           errlen);
...
int ds4_session_load_payload(ds4_session *s, FILE *fp, uint64_t payload_bytes, char *err, size_t errlen) {
    ...
    if (ds4_session_is_cpu(s)) {
        ...
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            ...
            if (payload_read_bytes(fp,
                                   layer->attn_comp_kv,
                                   (uint64_t)n_comp[il] * DS4_N_HEAD_DIM * sizeof(uint16_t),
                                   &remaining,
                                   err,
                                   errlen) != 0 ||
            ...
            if (ratio == 4) {
                if (payload_read_bytes(fp,
                                       layer->index_comp_kv,
                                       (uint64_t)n_index_comp[il] * DS4_N_INDEXER_HEAD_DIM * sizeof(uint16_t),
                                       &remaining,
                                       err,
                                       errlen) != 0 ||
    ...
    for (uint32_t il = 0; rc == 0 && il < DS4_N_LAYER; il++) {
        ...
        const uint32_t ratio = ds4_layer_compress_ratio(il);
        if (rc != 0 || ratio == 0) continue;
        /* Lever A (#34): see save site — restore the 608-byte FP8 rows. */
        rc = payload_read_tensor_span(fp,
                                      g->layer_attn_comp_cache[il],
                                      0,
                                      (uint64_t)n_comp[il] * DSV4_FP8_ATTN_ROW_BYTES,
                                      buf,
                                      DS4_SESSION_IO_CHUNK,
                                      &remaining,
                                      err,
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    if (opt->expert_mask_file && opt->expert_mask_file[0]) {
        e->expert_mask_file = ds4_strdup(opt->expert_mask_file);
    }
    ...
    if (graph_backend) {
        ...
        if (e->expert_mask &&
            !ds4_gpu_set_expert_mask(e->expert_mask, DS4_N_LAYER, DS4_N_EXPERT)) {
            fprintf(stderr, "ds4: failed to upload expert mask to %s backend\n",
                    ds4_backend_name(e->backend));
            ds4_engine_close(e);
            *out = NULL;
            return 1;
        }
        if (e->model.expert_shrunken && e->model.expert_orig_to_compact) {
            const uint32_t lc = e->model.expert_layer_count;
            const size_t   n  = (size_t)DS4_N_LAYER * DS4_N_EXPERT;
            int32_t *lut = xmalloc(n * sizeof(int32_t));
            for (size_t i = 0; i < n; i++) lut[i] = -1;
            for (uint32_t il = 0; il < DS4_N_LAYER && il < lc; il++) {
                const int16_t *row = e->model.expert_orig_to_compact + (size_t)il * DS4_N_EXPERT;
                int32_t       *out_row = lut + (size_t)il * DS4_N_EXPERT;
                for (uint32_t ex = 0; ex < DS4_N_EXPERT; ex++) out_row[ex] = row[ex];
            }
            const int ok = ds4_gpu_set_expert_keep_lut(lut, DS4_N_LAYER, DS4_N_EXPERT);
            free(lut);
            if (!ok) {
                fprintf(stderr, "ds4: failed to upload expert keep_map LUT to %s backend\n",
                        ds4_backend_name(e->backend));
                ds4_engine_close(e);
                *out = NULL;
                return 1;
...
void ds4_engine_close(ds4_engine *e) {
    ...
    if (g_cpu_expert_mask == e->expert_mask) g_cpu_expert_mask = NULL;
    free(e->expert_mask);
    free(e->expert_mask_file);
    free(e);
}
